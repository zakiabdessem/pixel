...
function MostReleventCards({ exclude }: any) {
  const [data, setData] = useState<Product[] | null>([]);

  async function fetch() {
    const response = await axios.get(
      `${API_URL}/products?limit=3&exclude=${exclude}`,
    );
        ...
        <>
          <div className="flex max-w-screen-xl px-4 py-8 mx-auto justify-start items-center w-full">
            <h1 className="sm:border-b-2 border-[#ECB176] max-w-2xl mb-4 text-2xl font-medium font-spotify tracking-tight leading-none text-gray-800">
              Autre
              <span className="text-[#ECB176]"> Articles</span>
            </h1>
          </div>
          <div className="flex max-w-screen-xl px-4 py-8 mx-auto justify-center items-center w-full">
...
export default MostReleventCards;
